...
<body>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["notes"]) && $_POST["notes"] == "notes") {
    ...
    if (!$link) {
      echo "Помилка з'єднання: " . mysqli_connect_error();
    } else {
      $created = date('Y-m-d H:i:s');
      $query = "INSERT INTO Notes (Title, Article, Created) VALUES ('$name', '$article', '$created')";

      if (mysqli_query($link, $query)) {
        echo "Новий запис успішно додано до таблиці Notes.";
  ...
  } else if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["comment"]) && $_POST["comment"] == "comment") {
    $author = $_POST["author"];
    ...
    if (!$link) {
      ...
      if (mysqli_query($link, $query)) {
        echo "Новий запис успішно додано до таблиці Comments.";
        header("Location: {$_SERVER['REQUEST_URI']}");
        exit();
      } else {
        echo "Помилка при виконанні запиту:: " . mysqli_error($link);
  ...
  } else if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_comment"]) && $_POST["delete_comment"] == "delete") {
